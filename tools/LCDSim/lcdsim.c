...
LCDSim* LCDSim_Create(SDL_Surface *screen, int x, int y, const char* base_path) {
    ...
    line_rect.y = 70;
    line_rect.h = 3;
    for (int x = 0; x < 331; x += PIXEL_DIM) {
        line_rect.x = x;
        line_rect.w = PIXEL_DIM;
        SDL_FillRect(self->gu.temp_screen, &line_rect, SDL_MapRGB(self->gu.temp_screen->format, 0, 255, 0));
    }
...
void HD44780_Init(HD44780 *self, const char* base_path) {
    ...
    for (int row = 0; row < 8; ++row)
        self->CGROM['H'][row] = 0b11111;  // Make "H" a solid block for test
...
void GraphicUnit_Init_old(GraphicUnit *self, const char* base_path) {
    ...
    for (i = 0; i < 2; i++)
        ...
        self->color[i] = SDL_CreateRGBSurface(0, PIXEL_DIM, PIXEL_DIM, 32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000);
...
void GraphicUnit_Init(GraphicUnit *self, const char* base_path) {
    ...
    // Set color[0] = white background
    SDL_FillRect(self->color[0], NULL, SDL_MapRGB(self->color[0]->format, 255, 255, 255));

    // Set color[1] = green character
    SDL_FillRect(self->color[1], NULL, SDL_MapRGB(self->color[1]->format, 0, 255, 0));
